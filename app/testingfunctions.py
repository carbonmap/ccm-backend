...
## Function 4: SELECT numb_value FROM entity_property WHERE is_numeric=1 AND id=id_variable UNION SELECT str_value FROM entity_property WHERE is_numeric=0 AND id=id_variable;
...
answer = sqliteExecute("db.sqlite", "SELECT entity_id FROM entity_to_subentity WHERE subentity_id=?", (id, ))
